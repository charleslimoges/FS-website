import { Bed, Bath, Maximize2, Car, PawPrint, Sofa, Zap, Tag, HardHat } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        {unit.building_name && (
          <p className="text-xs text-gray-400 mb-1 line-clamp-1">{unit.building_name}</p>
        )}

        <div className="flex items-start justify-between gap-2 mb-3">
          <h3 className="font-semibold text-brand-navy text-base leading-tight">
            Unit {unit.unit_number}
          </h3>
          <span className="text-brand-blue font-bold text-lg leading-tight whitespace-nowrap">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          ...
          {unit.utilities_included && (
            <span className="flex items-center gap-1 bg-violet-50 text-violet-700 text-xs px-2.5 py-1 rounded-lg border border-violet-100">
              <Zap className="w-3 h-3" /> Utilities Incl.
            </span>
          )}
        </div>
